...
| コンテナ | 用途 | 管理者 |
|----------|------|--------|
| `GTM-MWSK9GM8` | 既存の計測（GA4ページビュー等） | 社内共有 |
| `GTM-PVHHML82` | ファネル計測 + A/Bテスト（本ガイドの設定対象） | 自身で管理 |

> **TODO:** 第2コンテナのIDは、GTM管理画面でコンテナ作成後に置き換えてください。
...
window.dataLayer.push({
  ...
  cta_location: 'hero',        // hero / merit / pain_solution / service_detail / simulator / final_cta / header / header_mobile / sticky
});
```
...
ryohi-lp 用の GA4 プロパティを新規作成します。

### 手順

1. [GA4 管理画面](https://analytics.google.com/) にログイン
2. 左下の **歯車アイコン（管理）** をクリック
3. アカウント列で既存のアカウント（社宅LP用と同じアカウント）を選択
4. プロパティ列の **「+ プロパティを作成」** をクリック
5. プロパティ名: `PLEX 出張旅費規程 LP`
6. レポートのタイムゾーン: `日本`、通貨: `日本円（JPY）`
7. ビジネス情報を入力して **「作成」**
8. **データストリーム** → **「ウェブ」** を選択
9. ウェブサイトのURL: デプロイ先のドメインを入力
10. ストリーム名: `ryohi-lp`
11. **「ストリームを作成」** → 表示される **測定 ID（`G-40YNQWTLNK`）** をメモ

> この測定 ID を GTM の GA4 タグで使用します。

### カスタムディメンションの作成

GA4 側でイベントパラメータをレポートやエクスプロレーションで使えるようにするため、カスタムディメンションを登録します。

1. プロパティ列の **カスタム定義** をクリック
2. **カスタムディメンション** タブを選択し、**「カスタムディメンションを作成」** をクリック
...
以下の設定はすべて **ファネル計測 + A/Bテスト用コンテナ**（`GTM-PVHHML82`）で行います。
...
4. **タグ ID** に GA4 の測定 ID（`G-40YNQWTLNK`）を入力
5. **トリガー** → **「All Pages」** を選択
6. **「保存」**

> **注意:** 既存コンテナ（`GTM-MWSK9GM8`）で既にGA4のページビュー計測をしている場合、新コンテナでの Google タグは **不要** です（ページビューが二重計測されるため）。その場合は、手順2-4のGA4イベントタグで測定IDを直接指定してください。
...
3. バージョンの説明に以下を記載:
   ```
   ファネル計測の初期セットアップ
   - cta_click: CTAボタンクリック計測
   - form_submit: フォーム送信完了計測
   - conversion_page_view: サンクスページ表示（GA4セッション帰属維持のため必須）
   - schedule_click: 日程調整クリック計測
   - ab_test_impression: A/Bテストインプレッション計測
...
3. 対象ページに `ABTestImpression` コンポーネントを配置
4. 既存の `CE - ab_test_impression` トリガーが自動で発火
...
| イベントが二重に送信される | 既存コンテナ（GTM-MWSK9GM8）に同名のタグがないか確認 |
