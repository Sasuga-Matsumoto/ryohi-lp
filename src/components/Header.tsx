...
export default function Header({ variant = 'top' }: { variant?: Variant }) {
  ...
  const renderNav = () => {
    ...
    // contact, download, thanks-*, privacy
    return (
      <ul className="header-nav">
        <li><Link href="/">サービスサイト</Link></li>
        <li><TrackedLink href="/contact/" className="btn btn-primary" style={{ padding: '10px 22px', fontSize: '0.82rem', color: 'white' }} eventParams={{ form_type: 'contact', cta_location: 'header' }}>お問い合わせ</TrackedLink></li>
      </ul>
    );
  };
